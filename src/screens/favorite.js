...
const Favorite = ({ navigation }) => {
  ...
  const handleLike = async () => {
    ...
    await firestore().collection('users').doc(user.uid).collection('liked').add({ data: screens });
  };
  ...
  const handleSave = async () => {
    const user = auth().currentUser
    await firestore().collection('users').doc(user.uid).collection('Saved').doc().delete({ data: screens })
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const user = auth().currentUser;
      try {
        const snapshot = await firestore().collection('users').doc(user.uid).collection('Saved').get();
